...
/**
 * DeepSeek LLM provider using raw cURL (OpenAI-compatible API).
 *
 * Communicates with the DeepSeek Chat Completions API
 * (https://api.deepseek.com/v1/chat/completions) for both synchronous and
 * streaming text generation. DeepSeek uses the same request/response format
 * as OpenAI, so this provider closely mirrors OpenAILLMProvider with a
 * different base URL and authentication.
 *
 ...
 *
 * @param  string  $apiKey  DeepSeek API key for bearer authentication. Example: "sk-..."
 * @param  string  $model  LLM model identifier. Example: "deepseek-chat"
...
{
    /** @var string DeepSeek API key for bearer authentication */
    private string $apiKey;

    /** @var string LLM model name (e.g. "deepseek-chat") */
    ...
    /**
     * @param  string  $apiKey  DeepSeek API key. Example: "sk-..."
     * @param  string  $model  Model identifier. Example: "deepseek-chat"
     * @param  int  $timeout  Request timeout in seconds. Example: 60
     * @param  string  $baseUrl  API base URL. Example: "https://api.deepseek.com/v1"
     */
    ...
    /**
     * Perform a synchronous LLM completion via the DeepSeek Chat API.
     *
     * Sends system prompt and user prompt as messages, returns a structured
     * LLMResponse with content and token usage metadata. Follows the same
     ...
     * @param  array  $options  Override options. Example: ["temperature" => 0.3, "max_tokens" => 4096, "model" => "deepseek-chat"]
     * @return LLMResponse Response with content and token usage. Example: new LLMResponse("The answer is...", 150, 50, 200, "deepseek-chat", "stop")
     *
    ...
    public function complete(string $systemPrompt, string $userPrompt, array $options = []): LLMResponse
    {
        ...
        $choice = $response['choices'][0] ?? throw new \RuntimeException('DeepSeek API returned no choices');
    ...
    /**
     * Perform a streaming LLM completion via the DeepSeek Chat API.
     *
     * Uses cURL multi-handle to stream SSE events from the API.
     ...
     *
     * @return string Model name. Example: "deepseek-chat"
     */
    ...
    /**
     * Execute a single HTTP request to the DeepSeek Chat Completions API.
     *
     * Implements retry with exponential backoff (1s, 5s, 25s) for
     * transient server errors. Client errors (4xx) throw immediately.
     *
     * @param  array  $payload  Request payload. Example: ["model" => "deepseek-chat", "messages" => [...], "temperature" => 0.3]
    ...
    private function sendRequest(array $payload): array
    {
        ...
        for ($attempt = 1; $attempt <= $maxAttempts; $attempt++) {
            ...
            if ($httpCode >= 400 && $httpCode < 500) {
                throw new \RuntimeException("DeepSeek API returned HTTP {$httpCode}: {$response}");
            }
        ...
        if ($error !== '') {
            throw new \RuntimeException("DeepSeek API request failed after {$maxAttempts} attempts: {$error}");
        }

        throw new \RuntimeException("DeepSeek API returned HTTP {$httpCode} after {$maxAttempts} attempts: {$response}");
    }
}
